...
def sum_up_data(avg_data, std_data):
    ...
    for i in range(len(avg_data)):
        ...
        std_all.append(0.2 * np.sqrt(sum(map(lambda x: x * x, std_data[i]))))
...
if __name__ == "__main__":
    ...
    B25_avg_relative = B25_avg - B25_avg[0]
    B25_std_relative = np.sqrt(np.power(B25_std, 2) + B25_std[0] ** 2)
    ...
    plt.savefig("sasa_all.png", dpi=600)
