...
def extractcharge(mol2, formalcharge):
    ...
    if sum(charge) != float(formalcharge):
        residual = sum(charge) - float(formalcharge)
...
def replacecharge(top, newcharges):
    ...
    while count < len(newcharges):
        if top[atoms][0] == ";":
            ...
            top[atoms] = s + a[0] + s + a[1] + s + a[2] + s + a[3] + s +  a[4] + s + a[5] + s + a[6] + s + a[7] + '\n'
...
if __name__ == '__main__':
    ...
    updated_charges = extractcharge(mol2, args.charge)

    new_top = replacecharge(top, updated_charges)
